...
CRUD operations on a set of resources, including organizations, products, offers, entitlements, usage record groups for
meterting, etc.
...
value `openapi.ContextServerVariables` of type `map[string]string`.
...
using `openapi.ContextOperationServerIndices` and `openapi.ContextOperationServerVariables` context maps.
...
Note, each API key must be added to a map of `map[string]APIKey` where the key is: APIKeyAuth and passed in as the auth
...
Note, each API key must be added to a map of `map[string]APIKey` where the key is: BearerTokenAuth and passed in as the
